 ...
 *                   ○    ○    ○        ○
 *  如上：
 *   数据不是直接保存在节点上，是由节点在树上的位置确定的；结尾字符不一定都是叶子节点，可通过 isEnd标记实现。
 *   出根节点没有父节点外，其他节点都有且仅有一个父节点。
 */
public class Trie {
    private static final int R = 256;  // extended ascii

    private TrieNode root;  // 根节点，无实际意义
    private int size;   // Trie树中存储的字符串个数
    ...
    private TrieNode delete(TrieNode node, char[] text, int m) {
        ...
        if (m == text.length) {
            /**
             * 已处理完最后一个字符，不直接删除，设置 isEnd=false, 后续根据是否有后续节点判断是否删除
             */
            if (node.isEnd) size--;   // 表示Trie树存在输入字符串
            node.isEnd = false;
        } else {
    ...
    private TrieNode get(String text) {
        if (text == null || text.length() == 0) return root;
