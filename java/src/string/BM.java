 ...
 *   思路：
 *      1.BF: 字符串匹配时，模式串在主串上从左向右滑动，对应字符一一比较，当遇到不匹配字符时，向右滑动一个位置
 *      2.BM: Moore算法的优化思路主要在于，当遇到不匹配字符时，尽可能多地向右滑动，而不是只滑动一个位置
 *         1).不同于BF模式串与主串从左到右进行字符比较，BM从右到左比较（滑动依然是从左向右，只是字符比较从右向左），
 *            不匹配时，获得 bad character(bc), 模式串的对应下标 pi
 ...
 *
 *  time complexity:
 *    在最坏情况下，BM 算法的比较次数上限是 3n (<a href="https://dl.acm.org/citation.cfm?id=127830"></a>)
 *
 *  space complexity:
 *    坏字符数组: O(K) - K: 字符集个数，当字符集很大时消耗较大空间，为了降低空间复杂度可不使用坏字符规则（算法效率会下降）
 *    suffix数组: O(m)
 *    prefix数组: O(m)
 *
 *
...
public class BM {
     ...
     *    bc:  suffix[2] = 1, prefix[2] = false
     *   abc:  suffix[3] = 0, prefix[3] = true
     *  cbac:  suffix[4] = -1, prefix[4] = false
     * bcabc:  suffix[5] = -1, prefix[5] = false
     *
     * @param pattern 模式串
     * @param m   模式串长度
     * @param prefix
    ...
    private int match(String text, String pattern, int[] map, int[] suffix, boolean[] prefix, int m, int n) {
        for (int i = 0; i < n - m + 1; ) {
            ...
            bc_index = j;
            b_shift = bc_index - map[(int) text.charAt(bc_index + i)]; // bc_index + i 为坏字符在主串中的索引下标

            // 2. good suffix shift
            if (bc_index == m - 1) {   //no good suffix
                i += b_shift;
                continue;
            }

            int s_shift = 0;  // 由好后缀规则计算出的滑动距离
            int suf = m - 1 - bc_index;   // length of good suffix
            if (suffix[suf] != -1) {
                s_shift = m - suffix[suf] - suf;
            } else {
                int k;
                for (k = suf - 1; k > 0; k--) {
                    if (prefix[k]) {
