...
public class TopologicalSort {
     ...
     * Time Complexity: O(v + e) -- v: the number of vertices in this graph
     *                              e: the number of edges in this graph
     *
     * 拓扑排序可以检测有向图是否存在环：如果返回结果中的顶点个数小于图的顶点个数，表明图中存在环
     */
    public List<Integer> topologicalSort_1() {
     ...
     *   核心： 递归处理每个顶点。先将依赖于当前顶点（即指向当前顶点）的顶点输出（图的表示只保存当前顶点指向的顶点，所以要构造逆邻接表
     *         来保存指向当前顶点的所有顶点），再输出当前顶点
     * Time Complexity：O(v + e)
     */
    public List<Integer> topologicalSort_2() {
        List<Integer> result = new ArrayList<>();
